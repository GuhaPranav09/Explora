...
if ("explora",) not in out1:
    myc.execute("create database Explora")
myc.execute("use explora")
myc.execute("create table if not exists encrypted(File varchar(255), EnKey varchar(255))")
con.commit()
...
def refresh(queryNames):
    ...
    for i in range(len(fileNames)):
        try:
            ...
            # check file type
            extensions(fileTypes, fileNames, i)
...
def create_widgets(window):
    ...
    m.add_command(
        ...
        image=encrypt_photo,
        compound="left",
        command=encrypt_file_popup,
    )
    m.add_command(
        label="Decrypt selected",
        image=decrypt_photo,
        compound="left",
        command=decrypt_file_popup,
    )
    m.add_separator()
    m.add_command(
        label="Refresh",
    ...
    help_menu.add_command(
        label="Know Encryption Key", command=encryption_popup, image=info_photo, compound="left"
    )

    help_menu.add_command(
        label="Change Encryption Key", command=change_encryption_popup, image=info_photo, compound="left"
    )
    ...
    photo_ref.append(encrypt_photo)
    photo_ref.append(decrypt_photo)
    ...
    window.bind("<Control-Shift-F>", wrap_new_file)
...
def write_key(key):
    with open(file_path + "../res/key.txt", "w") as f:  # closes file automatically
        f.write(key)
...
def about_popup():  # popup window
    Messagebox.ok(
        message="Explora\nMade by: \n22BCE1462 Guha Pranav Yelchuru\n22BCE1473 Viraj Bipin Pradhan\n22BCE5168 Bala Adhithiya",
        title="About",
    )

def encryption_popup():
    Messagebox.ok(
        message="Encryption by FERNET\n\nKey: {}".format(Encryption_Password),
        title="Encyrption",
    )

def change_encryption_popup():
    global Encryption_Password
    old_password = Querybox.get_string(prompt="Old Password: ", title="Password change")
    if old_password==Encryption_Password:
        new_password = Querybox.get_string(prompt="New Password: ", title="Password change")
        Encryption_Password=new_password
        write_key(Encryption_Password)
        Messagebox.ok(
        message="Encryption Password successfully changed",
        title="info",
    )
        refresh([])
    else:
        Messagebox.show_error("Wrong Encryption Password!", "Error")
...
def encrypt_file_popup():
    ...
    if items.focus() != "":  # if there is a focused item
        ...
        Messagebox.show_info(
            message="There is no selected file.", title="Info"
        )

def encrypt_file():
    path = os.getcwd() + "/" + selectedItem
    if os.path.isfile(path):
        myc.execute("SELECT * FROM encrypted WHERE File = %s", (path,))
        existing_record = myc.fetchone()
                
        if not existing_record:
            password = Querybox.get_string(prompt="Encryption Password: ", title="Enter Password")
            if password==Encryption_Password:
                key=Fernet.generate_key()
                myc.execute("insert into encrypted values(%s, %s)",(path, key))
                con.commit()
                ...
                Messagebox.show_info(message="Encryption Successful.", title="Info")
            else:
                Messagebox.show_error("Wrong Password!", "Error")
        else:
            Messagebox.show_info(
            message="File is already encrypted.", title="Info"
        )

    else: 
        Messagebox.show_info(
            message="Only files can be encrypted/decrypted.", title="Info"
        )

def decrypt_file_popup():
    global items
    if items.focus() != "":  # if there is a focused item
        answer = Messagebox.yesno(
            message="Are you sure?\nThis file will be decrypted.",
            alert=True,
        )
        if answer == "Yes":
            decrypt_file()
...
def decrypt_file():
    path = os.getcwd() + "/" + selectedItem
    if os.path.isfile(path):
        myc.execute("SELECT EnKey FROM encrypted WHERE File = %s", (path,))
        existing_record = myc.fetchone()
                
        if existing_record:
            password = Querybox.get_string(prompt="Password: ", title="Enter Encryption Password")
            if password==Encryption_Password:
                with open(path, "rb") as file:
                    encrypted_data = file.read()
                key = existing_record[0]
                cipher_suite = Fernet(key)
                decrypted_data = cipher_suite.decrypt(encrypted_data)
                with open(path, "wb") as file:
                    file.write(decrypted_data)
                myc.execute("delete from encrypted where file=%s",(path,))
                con.commit()
                Messagebox.show_info(message="Decryption Successful.", title="Info")
            else:
                Messagebox.show_error("Wrong Password!", "Error")
        else:
            Messagebox.show_info(
            message="File is not encrypted.", title="Info"
        )
    else: 
        Messagebox.show_info(
            message="Only files can be encrypted/decrypted.", title="Info"
...
def read_key():
    global Encryption_Password, file_path
    with open(file_path + "../res/key.txt") as f:  # closes file automatically
        Encryption_Password = f.readline()
    if Encryption_Password == "":  # if theme.txt is empty, set default theme
        Encryption_Password = "explora"
